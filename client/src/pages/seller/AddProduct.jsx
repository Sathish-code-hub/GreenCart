...
const AddProduct = () => {
    ...
    return  (
        <div className="no-scrollbar flex-1 h-[95vh] overflow-y-scroll flex flex-col justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-3 mt-2">
                        {Array(4).fill('').map((_, index) => (
                            <label key={index} htmlFor={`image${index}`}>
                                ...
                                src={files[index] ? URL.createObjectURL(files[index]) : assets.upload_area}  alt={`Upload ${index}`} width={100} height={100} />
                            </label>
                        ))}
                    </div>
                </div>
            ...
            {loading && (
                <div className="fixed top-0 left-0 w-full h-full bg-gray-600 bg-opacity-40 z-[9999] flex items-center justify-center">
                    <div className="bg-white px-6 py-4 rounded-md shadow-lg text-center flex flex-col items-center">
                        <svg className="w-10 h-10 text-primary animate-spin mb-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
